                ...
                try:
                    ...
                    if problem.status in [cp.OPTIMAL, cp.OPTIMAL_INACCURATE]:
                        ...
                        return {
                            ...
                            "cvar": (
                                var.value
                                + (1 / (self.alpha * len(returns))) * sum(s.value)
                            )
                            / 10000,
            ...
            if problem.status in [cp.OPTIMAL, cp.OPTIMAL_INACCURATE]:
                ...
                return {
                    ...
                    * np.sqrt(trading_days),
                    ...
                    "cvar": (
                        (var.value + (1 / (self.alpha * len(returns))) * sum(s.value))
                        / 10000
                    )
                    * np.sqrt(trading_days),
                    "solver_used": config["solver"],
...
def main():
    ...
    parser.add_argument(
        "--alpha", type=float, default=0.05, help="CVaR confidence level (0.01-0.5)"
    )
    parser.add_argument(
        "--cash_sleeve", type=float, default=0.05, help="Cash allocation (0-0.5)"
    )
    parser.add_argument(
        "--max_weight", type=float, default=0.3, help="Max asset weight (0.05-1.0)"
    )
    parser.add_argument(
        "--min_weight", type=float, default=0.0, help="Min asset weight (0-0.2)"
    )
    parser.add_argument(
        "--data_path", type=str, default="data/returns.csv", help="Path to returns data"
    )
    ...
    try:
        ...
        if result:
            ...
            # Save results
            pd.DataFrame(
                {"Asset": returns.columns, "Weight": result["weights"]}

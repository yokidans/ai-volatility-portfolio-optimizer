...
class FeatureBuilder:
    ...
    def _create_volatility_features(self, features, prices):
        ...
        try:
            ...
            features["garch_vol"] = (
                features["scaled_returns"].ewm(span=21, min_periods=5).std().div(100)
            )
        ...
        if len(returns) > 30:  # Sufficient for meaningful analysis
            try:
                freq, power = periodogram(
                    returns - returns.mean(), detrend="linear", scaling="spectrum"
                )
                ...
                features.loc[valid_idx, "freq_power_weekly"] = np.sum(
                    power[weekly_band]
                )
                features.loc[valid_idx, "freq_power_monthly"] = np.sum(
                    power[monthly_band]
                )
    ...
    def _create_derived_features(self, features):
        ...
        features["vol_ratio_short_long"] = features["vol_5d"] / features[
            "vol_63d"
        ].replace(0, np.nan)
        features["vol_regime"] = (features["vol_5d"] > features["vol_21d"]).astype(int)

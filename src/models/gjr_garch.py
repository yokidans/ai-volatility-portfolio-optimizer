...
class GJRGARCHModel:
        ...
        try:
            ...
            self.diagnostics = {
                'residuals_normality': {
                    ...
                    'shapiro': {'statistic': shapiro_test[0], 'pvalue': shapiro_test[1]}
                },
    ...
    def _check_explosive_volatility(self) -> bool:
        ...
        beta = self.results.params.get('beta[1]', 0)
        return beta >= 0.99  # Very high persistence may indicate near-explosive volatility
    ...
    def forecast_insample(self) -> pd.Series:
        ...
        if not self.results:
            raise ValueError("Model not fitted. Call fit() first.")
        return pd.Series(self.results.conditional_volatility / 100, 
    ...
    def rolling_forecast(self, 
        ...
        def _process_window(i):
            try:
                ...
                if i % 10 == 0 or i == window:
                    ...
                    logger.info(f"Processing {progress:.1f}% complete ({i-window}/{len(returns)-window})")
        ...
        try:
            ...
            if not forecasts.empty:
                ...
                forecasts['rolling_rmse'] = np.sqrt(forecasts['squared_error'].rolling(21).mean())
    ...
    def plot_forecasts(self, forecasts: pd.DataFrame, save_path: Optional[str] = None) -> None:
        ...
        plt.subplot(2, 1, 1)
        plt.plot(forecasts.index, forecasts['actual_vol'], 
                label='Actual Volatility', color='blue', alpha=0.6)
        plt.plot(forecasts.index, forecasts['sigma'], 
        ...
        if 'rolling_mae' in forecasts.columns:
            plt.plot(forecasts.index, forecasts['rolling_mae'], 
                    label='21-Day Rolling MAE', color='green')
        if 'rolling_rmse' in forecasts.columns:
            plt.plot(forecasts.index, forecasts['rolling_rmse'], 
                    label='21-Day Rolling RMSE', color='purple')
    ...
    def save_results(self, forecasts: pd.DataFrame, path: str) -> None:
        ...
        results = {
            ...
            'parameters': self.results.params.to_dict() if self.results is not None else {},
            'diagnostics': self.diagnostics,
            'forecasts': forecasts.reset_index().to_dict(orient='records') if not forecasts.empty else [],
            'metrics': {
                'mae': float(forecasts['abs_error'].mean()) if not forecasts.empty else None,
                'rmse': float(np.sqrt((forecasts['error']**2).mean())) if not forecasts.empty else None,
                'mse': float((forecasts['error']**2).mean()) if not forecasts.empty else None,
                'last_rolling_mae': float(forecasts['rolling_mae'].iloc[-1]) if not forecasts.empty and 'rolling_mae' in forecasts.columns else None,
                'last_rolling_rmse': float(forecasts['rolling_rmse'].iloc[-1]) if not forecasts.empty and 'rolling_rmse' in forecasts.columns else None
            }
        }
...
if __name__ == "__main__":
    ...
    parser.add_argument("--n_jobs", type=int, default=1, help="Number of parallel jobs (1 for reliability)")
    ...
    try:
        ...
        forecasts = model.rolling_forecast(returns, window=args.window, n_jobs=args.n_jobs)
        ...
        if not forecasts.empty:
            ...
            metrics = {
                ...
                'Last_21D_MAE': forecasts['rolling_mae'].iloc[-1] if 'rolling_mae' in forecasts.columns else float('nan'),
                'Last_21D_RMSE': forecasts['rolling_rmse'].iloc[-1] if 'rolling_rmse' in forecasts.columns else float('nan')
            }

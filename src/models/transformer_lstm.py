...
class FinancialDataset(Dataset):
    def __init__(self, X: np.ndarray, y: np.ndarray, window_size: int = 63):
        # Input validation
        if len(X.shape) != 3:
            raise ValueError(f"X must be 3D array (samples, timesteps, features), got {len(X.shape)}D")
        ...
        if self.window_size > X.shape[1]:
            raise ValueError(f"window_size {window_size} cannot be larger than sequence length {X.shape[1]}")
...
def quantile_loss(y_true: torch.Tensor, y_pred: torch.Tensor, quantiles: List[float]) -> torch.Tensor:
    ...
    if y_pred.dim() != 2 or y_pred.shape[1] != len(quantiles):
        raise ValueError(f"y_pred must be (batch_size, {len(quantiles)}), got {y_pred.shape}")
...
def train_model(args):
    ...
    model_name = f"transformer_lstm_{args.input_size}feat_{args.hidden_size}hid_{timestamp}"
    ...
    for epoch in range(args.epochs):
        ...
        if (epoch + 1) % 5 == 0 or epoch == args.epochs - 1:
            pd.DataFrame(metrics).to_csv(
                os.path.join(report_dir, "training_metrics.csv"),
                index=False
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Transformer-LSTM for Financial Forecasting')
    parser.add_argument("--input_size", type=int, required=True, help="Number of input features")
    parser.add_argument("--epochs", type=int, default=50, help="Number of training epochs")
    parser.add_argument("--batch_size", type=int, default=32, help="Batch size for training")
    parser.add_argument("--hidden_size", type=int, default=128, help="Hidden size of the model")
    parser.add_argument("--num_layers", type=int, default=3, help="Number of layers in Transformer and LSTM")

...
class DCCGARCH:
    ...
    def __init__(self, univariate_garch_params: Optional[Dict] = None, dcc_params: Optional[Dict] = None):
    ...
    def _fit_univariate_models(self, returns: pd.DataFrame) -> None:
        ...
        for asset in returns.columns:
            with warnings.catch_warnings():
                ...
                model = arch_model(
                    returns[asset],
                    **self.univariate_garch_params
                )
                results = model.fit(disp='off')
                self.univariate_models[asset] = results
                standardized_residuals[asset] = results.resid / results.conditional_volatility
    ...
    def _fit_dcc_model(self) -> None:
        ...
        for t in range(1, n_obs + 1):
            Qt[t] = Qbar * (1 - a - b) + a * np.outer(residuals_array[t-1], residuals_array[t-1]) + b * Qt[t-1]
